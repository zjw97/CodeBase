...
class Evaluator:
    ...
    def evaluate(self):
        ...
        for dataset, clip, future in tasks:
            for i, metric in enumerate(future.result().keys()):
                tmp = future.result()[metric]
                metrics[metric].append(sum(tmp) / len(tmp))

        for metric, value in metrics.items():
            print(metric, sum(value) / len(value))
    ...
    def evaluate_worker(self, dataset, clip, position):
        ...
        for i, framename in enumerate(
            ...
            true_pha = cv2.imread(os.path.join(self.args.true_dir, dataset, clip, 'pha', framename),
                                  cv2.IMREAD_GRAYSCALE).astype(np.float32) / 255
            pred_pha = cv2.imread(os.path.join(self.args.pred_dir, dataset, clip, 'pha', framename),
                                  cv2.IMREAD_GRAYSCALE).astype(np.float32) / 255
            ...
            if 'fgr_mse' in self.args.metrics or 'fgr_mad' in self.args.metrics:
                true_fgr = cv2.imread(os.path.join(self.args.true_dir, dataset, clip, 'fgr', framename),
                                      cv2.IMREAD_COLOR).astype(np.float32) / 255
                pred_fgr = cv2.imread(os.path.join(self.args.pred_dir, dataset, clip, 'fgr', framename),
                                      cv2.IMREAD_COLOR).astype(np.float32) / 255
                true_msk = true_pha > 0
